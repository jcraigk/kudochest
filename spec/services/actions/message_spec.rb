...
RSpec.describe Actions::Message do
  ...
  shared_examples 'platform parity' do
    ...
    context 'when text includes single valid inline emoji with valid user' do
      let(:text) { "hello <#{user_mention}> :#{team.karma_emoji}: #{note}" }
      let(:mentions) do
        [OpenStruct.new(rid: user_mention, quantity: 1, topic_id: nil)]
      end
    ...
    context 'when text includes single valid inline emoji with int suffix' do
      let(:text) { "hello <#{user_mention}> :#{team.karma_emoji}: 2 #{note}" }
      let(:mentions) do
        [OpenStruct.new(rid: user_mention, quantity: 2, topic_id: nil)]
      end
    ...
    context 'when text includes single valid inline emoji with int prefix' do
      let(:text) { "hello <#{user_mention}> 2 :#{team.karma_emoji}: #{note}" }
      let(:mentions) do
        [OpenStruct.new(rid: user_mention, quantity: 2, topic_id: nil)]
      end
    ...
    context 'when text includes multiple valid inline emoji with int suffix' do
      let(:text) do
        <<~TEXT.chomp
          hello <#{user_mention}> :#{team.karma_emoji}: :#{team.karma_emoji}: :#{team.karma_emoji}: 2 #{note}
        TEXT
      end
      let(:mentions) do
    ...
    context 'when text includes multiple mixed inline emoji with valid user' do
      let(:text) do
        "hello <#{user_mention}> :#{team.karma_emoji}: :invalid_emoji::#{team.karma_emoji}: #{note}"
      end
      let(:mentions) do
        [OpenStruct.new(rid: user_mention, quantity: 2, topic_id: nil)]
      end
